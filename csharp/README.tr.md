...
Bu proje, domain ortamında **sadece izinli tek kullanıcı** hesabı ile, toplu şekilde HTML dosyalarını PDF'e çevirmek için hazırlanmıştır.
...
Kurulum yapılacak Windows sunucuda şunlar olmalı:
- Windows Server (domain'e join edilmiş)
...
Uygulama:
1. Makinenin domain bilgisini doğrular.
2. Oturum açmış kullanıcının `AllowedUser` ile birebir eşleştiğini kontrol eder.
3. Klasördeki `.html` ve `.htm` dosyalarını PDF'e dönüştürür.
4. Sonucu log dosyasına yazar.
...
- **Domain hatası**: Sunucu domain'e bağlı değil veya `AllowedDomain` yanlış.
...
- **DomainAuthorizationService**: Domain + tek kullanıcı yetkisini doğrular.
